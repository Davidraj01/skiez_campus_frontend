    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0f172a 0%,#1e293b 60%,#0f172a 100%)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div style={{ background:"#fff", borderRadius:20, padding:40, width:380, boxShadow:"0 30px 80px rgba(0,0,0,0.4)" }}>
        <div style={{ textAlign:"center", marginBottom:28 }}>
          ...
          <p style={{ color:"#64748b", margin:0, fontSize:13 }}>IT Company Management · 40 Members · 8 Teams</p>
        </div>
  ...
  return (
    <div style={{ padding:28, overflowY:"auto", height:"100%" }}>
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ color:"#64748b", margin:0, fontSize:14 }}>Full access to all 8 teams. Team members only see their own workspace.</p>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14, marginBottom:28 }}>
        {[{label:"Total Teams",value:8,icon:"🏢",color:"#6366f1"},{label:"Team Members",value:40,icon:"👥",color:"#10b981"},{label:"Total Users (DB)",value:stats.users,icon:"💾",color:"#3b82f6"}].map((s,i)=>(
  ...
  return (
    <div style={{ display:"flex", height:"100vh", fontFamily:"'Inter',-apple-system,sans-serif", background:"#f8fafc" }}>
      ...
      <div style={{ width:230, background:"#0f172a", display:"flex", flexDirection:"column", flexShrink:0 }}>
        <div style={{ padding:"20px 16px 14px" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
            ...
            <div><div style={{ color:"#fff", fontWeight:800, fontSize:14 }}>TeamBase Pro</div><div style={{ color:"#475569", fontSize:10 }}>40 Members · 8 Teams</div></div>
          </div>
        ...
        <nav style={{ flex:1, padding:"0 10px", overflowY:"auto" }}>
          {user.role==="admin" && (
            <>
              <button onClick={()=>{ setAdminTab("dashboard"); setActiveTeam(null); }} style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"9px 10px", marginBottom:4, border:"none", borderRadius:8, background:!activeTeam&&adminTab==="dashboard"?"#1e293b":"transparent", color:!activeTeam&&adminTab==="dashboard"?"#fff":"#94a3b8", cursor:"pointer", fontSize:13, fontWeight:600, textAlign:"left" }}>🏠 Dashboard</button>
              <button onClick={()=>{ setAdminTab("users"); setActiveTeam(null); }} style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"9px 10px", marginBottom:8, border:"none", borderRadius:8, background:!activeTeam&&adminTab==="users"?"#1e293b":"transparent", color:!activeTeam&&adminTab==="users"?"#fff":"#94a3b8", cursor:"pointer", fontSize:13, fontWeight:600, textAlign:"left" }}>👥 User Management</button>
            </>
          )}
          ...
          {visibleTeams.map(team=>(
            <button key={team.id} onClick={()=>{ setActiveTeam(team.id); setTab("tasks"); }} style={{ width:"100%", display:"flex", alignItems:"center", gap:8, padding:"9px 10px", marginBottom:3, border:"none", borderRadius:8, background:activeTeam===team.id?"#1e293b":"transparent", color:activeTeam===team.id?"#fff":"#94a3b8", cursor:"pointer", fontSize:13, fontWeight:activeTeam===team.id?700:400, textAlign:"left" }}>
              <span>{team.icon}</span>
        ...
        <div style={{ padding:"10px 10px 16px", borderTop:"1px solid #1e293b", display:"flex", flexDirection:"column", gap:4 }}>
          <button onClick={()=>setShowChangePw(true)} style={{ background:"none", border:"none", color:"#64748b", cursor:"pointer", fontSize:12, textAlign:"left", padding:"6px 10px", borderRadius:6 }}>🔐 Change Password</button>
          <button onClick={logout} style={{ background:"none", border:"none", color:"#64748b", cursor:"pointer", fontSize:12, textAlign:"left", padding:"6px 10px", borderRadius:6 }}>← Logout</button>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        {/* Top bar */}
        <div style={{ background:"#fff", padding:"12px 24px", borderBottom:"1px solid #f1f5f9", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          {activeTeam && (
            <div style={{ display:"flex", gap:4, background:"#f1f5f9", borderRadius:10, padding:4 }}>
              {[["tasks","📋 Tasks"],["chat","💬 Chat"],["members","👥 Members"]].map(([k,l])=>(
                <button key={k} onClick={()=>setTab(k)} style={{ padding:"7px 14px", border:"none", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:tab===k?700:500, background:tab===k?"#fff":"transparent", color:tab===k?"#6366f1":"#64748b", boxShadow:tab===k?"0 1px 4px rgba(0,0,0,0.08)":"none" }}>{l}</button>
              ))}
            </div>
          )}
        </div>
